...
async function computeGnosisSafeAddress() {
  ...
  try {
    ...
    const factories = [
      { name: 'Gnosis Safe Factory', contract: gnosisSafeFactory, address: GNOSIS_SAFE_PROXY_FACTORY },
      { name: 'Polymarket Proxy Factory', contract: polymarketProxyFactory, address: POLYMARKET_PROXY_FACTORY }
    ];
